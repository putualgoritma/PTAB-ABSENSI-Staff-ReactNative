...
import {ScrollView, StyleSheet, View,ImageBackground, Image, Text,Dimensions, Modal, Button,TouchableOpacity, TouchableHighlight, Alert } from 'react-native';
...
const ShowDetail = () => {
  ...
  setLoading(true)
  let d1 = [];
  API.listFile(id,TOKEN).then((result) => {
      console.log('data',result.data)
      // console.log('datasss',JSON.parse(result.data.image))

      setImg(result.data)
    //   simpan foto
    result.data && result.data != "" && result.data.map((item, index) => {
      d1.push({
        url: `https://simpletabadmin.ptab-vps.com/images/RequestFile/` + `${String(item.image).replace('public/', '')}?time="${new Date()}`,
              })
          })
          console.log('gggs',d1)
          setImages(d1);
...
useEffect(() => {
  ...
  return (
   ...
   <Text style ={{ color : '#000000', fontSize : 18 }}>{route.params.type == 'sick' ? "Sakit" : 'Izin' }</Text>
   <Text style ={{ color : '#000000', fontSize : 16, marginBottom : windowHeight*0.05 }}>{route.params.start} s/d {route.params.end}</Text>
...

{
                   img.map((item, index) => {
                     return (
  <View style={{ backgroundColor : '#FFFFFF', marginLeft : 'auto', marginRight : 'auto', width : windowWidht*0.95, paddingBottom : windowHeight*0.01 }}>
<Text>{item.id}</Text>
    <DataView title='Bukti' />
             ...
             >
                 <ImageViewer imageUrls={[{url: `https://simpletabadmin.ptab-vps.com/images/RequestFile/` + `${String(item.image).replace('public/', '')}?time="${new Date()}`}]}/>
             </Modal>
             <TouchableHighlight onPress ={() =>{ setShowImage(true);console.log('tess',images);}}>
             ...
             <ImageBackground style={{ height : 220, width : 280}} >
                         ...
                         <Image 
                             key={index} 
                             style={{height : 220, width : 280, marginVertical : 10, marginLeft : 'auto', marginRight : 'auto'}} 
                             source = {{uri : `https://simpletabadmin.ptab-vps.com/images/RequestFile/` + `${String(item.image).replace('public/', '')}`}}
             ...
             <TouchableOpacity style={{ marginLeft : 'auto', marginRight : windowWidht*0.05, width : windowWidht*0.3, padding : 2, backgroundColor : '#d72503', borderRadius : 2 }} onPress = { ()=>{deleteImage(item.id)}  }>
