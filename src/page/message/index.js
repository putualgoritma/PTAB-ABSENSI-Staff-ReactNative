...
setData(newArray)
              ...
              console.log('tessaaaaaas')
                API.readMessage({id : id}, TOKEN).then((result)=>{
                  console.log('tessaaaaaas')
                    // resetData()
                }).catch((e) => {
                    console.log(e);
      ...
      const renderItem = ({ item }) => {
        ...
        if(item.type == "check" && item.status != "read"){
          return (
            ...
            <TouchableOpacity style ={styles.data} onPress={()=>{navigation.replace("Check_Staff", {id : item.id, lng : route.params.lng, lat : route.params.lat, radius : route.params.radius })}}>
                  <Text style={[{fontSize : 16, fontWeight : 'bold', color : '#000000', marginVertical : windowHeight*0.01}]}>{item.type}</Text>
...
return(
  <TouchableOpacity style ={styles.data} onPress={()=>{read(item.id, item.message)}}>
  <Text style={[{fontSize : 16, fontWeight : 'bold', color : '#000000', marginVertical : windowHeight*0.01}]}>{item.type}</Text>

{/* <Text style={styles.title}>Shift </Text> */}
<Text style={{ color : "#000000" }}>{item.created_at}<Text style={{ color : "#ff0000" }}>*</Text></Text>
